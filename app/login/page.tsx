...
export default function LoginPage() {
  ...
  const { isClerkEnabled, login, loginWithProvider, isAuthenticated, isLoading: authLoading } = useAuthContext();
  ...
  return (
    <div className="w-full max-w-md mx-auto my-8 px-4 flex flex-col justify-center min-h-[70vh]">
      {/* TODO: Add your Clerk API Keys and Turnstile site keys in your .env.local file:
        ...
        <div className="relative bg-card-bg/80 border border-card-border backdrop-blur-xl p-8 rounded-2xl flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              disabled={isLoading}
              className="flex items-center justify-center gap-2.5 py-2.5 px-4 rounded-xl border border-card-border bg-card-muted hover:bg-card-border hover:text-accent font-medium text-xs transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed text-foreground"
            >
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between text-xs py-1">
              <label className="flex items-center gap-2 font-medium text-untyped cursor-pointer select-none">
                <input
                  type="checkbox"
                  checked={rememberMe}
                  onChange={(e) => setRememberMe(e.target.checked)}
                  disabled={isLoading}
                  className="rounded border-card-border bg-card-muted text-accent focus:ring-accent/30 w-4 h-4 transition-all"
                />
